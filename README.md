...
- [X] Explain what a token is used for.
  ...
  Some parts of an app are restricted to users who have created an account on that platform. Once a user enters the proper credentials, the app's server will issue a token to grant that user acces to the protected parts of the app. A token is like a key that gives the user access to the locked portions of an app. a token is used in the server requests to ensure the user has the authorization to access restricted parts of an app.


- [X] What steps can you take in your web apps to keep your data secure?

  To make apps more secure, a developer can create PrivateRoutes that restrict access to certain portions of an app that only registered users can access. These PrivateRoutes will check for specific tokens, generated by the app's server, to ensure that only registered users can access portions of an app that might have sensitive data.


- [X] Describe how web servers work.

  A "web server" can refer to either a computer that stores the code for a website, or the the software that runs on that computer. When a user types in a URL, their computer will make a request to that site's server. That server hardware takes the request, the server software determines what data is being requested, then the server sends back a response with the corresponding data and code for that particular URL.


- [X] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

  Create = axios.post
  When we want to add new data that we would like to be stored on a remote server, we use a Post Request.

  Read = axios.get
  When we want to access data stored on a server, but not manipulate it, we use a Get Request.

  Update = axios.put
  When we want to alter existing data on a server, we use a Put Request

  Delete = axios.delete
  When we want to delete existing data that is stored on a server, we use the Delete Request
